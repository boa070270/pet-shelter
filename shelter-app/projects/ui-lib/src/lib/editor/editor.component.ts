...
export class EditorComponent implements OnInit {
  ...
  showDesigner(): void {
    ...
    this.editorElement.nativeElement.innerHTML = this.source;
    // @ts-ignore
  ...
  showSource(): void {
    this.source = this.editorElement.nativeElement.innerHTML;
    this.slide.to(1);
  }
  ...
  drop(ev): void {
    ...
    if (types.length < 3 && types.indexOf('text/plain') > -1) {
      return;
    }
